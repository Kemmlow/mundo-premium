...
import dev.knoxy.core.fake.hook.IInjectHook;
...
public class HCallbackProxy implements IInjectHook, Handler.Callback {
    ...
    private Handler getH() {
        Object currentActivityThread = KnoxyCoreCore.mainThread();
    ...
    private boolean handleLaunchActivity(Object client) {
        ...
        if (activityInfo != null) {
            if (BActivityThread.getAppConfig() == null) {
                KnoxyCoreCore.getBActivityManager().restartProcess(activityInfo.packageName, activityInfo.processName, stubRecord.mUserId);

                Intent launchIntentForPackage = KnoxyCoreCore.getBPackageManager().getLaunchIntentForPackage(activityInfo.packageName, stubRecord.mUserId);
            ...
            KnoxyCoreCore.getBActivityManager().onActivityCreated(taskId, token, stubRecord.mActivityRecord);

            if (BuildCompat.isS()) {
                Object record = BRActivityThread.get(KnoxyCoreCore.mainThread()).getLaunchingActivity(token);
    ...
    private boolean handleCreateService(Object data) {
        if (BActivityThread.getAppConfig() != null) {
            ...
            if (!serviceInfo.name.equals(ProxyManifest.getProxyService(BActivityThread.getAppPid()))
                ...
                KnoxyCoreCore.getBActivityManager().startService(intent, null, false, BActivityThread.getUserId());
                return true;
            }
        }
        return false;
