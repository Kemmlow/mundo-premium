...
import dev.knoxy.core.entity.AppConfig;
import dev.knoxy.core.entity.am.RunningAppProcessInfo;
import dev.knoxy.core.entity.am.RunningServiceInfo;
import dev.knoxy.core.fake.delegate.ContentProviderDelegate;
import dev.knoxy.core.fake.delegate.InnerReceiverDelegate;
import dev.knoxy.core.fake.delegate.ServiceConnectionDelegate;
import dev.knoxy.core.fake.frameworks.BActivityManager;
import dev.knoxy.core.fake.frameworks.BPackageManager;
...
import dev.knoxy.core.fake.hook.ScanClass;
import dev.knoxy.core.fake.service.base.PkgMethodProxy;
import dev.knoxy.core.fake.service.context.providers.ContentProviderStub;
import dev.knoxy.core.proxy.ProxyManifest;
import dev.knoxy.core.proxy.record.ProxyBroadcastRecord;
import dev.knoxy.core.proxy.record.ProxyPendingRecord;
import dev.knoxy.core.utils.MethodParameterUtils;
import dev.knoxy.core.utils.Reflector;
import dev.knoxy.core.utils.compat.ActivityManagerCompat;
import dev.knoxy.core.utils.compat.BuildCompat;
import dev.knoxy.core.utils.compat.ParceledListSliceCompat;
import dev.knoxy.core.utils.compat.TaskDescriptionCompat;
    ...
    public static class GetContentProvider extends MethodHook {
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Exception {
            ...
            if (auth instanceof String) {
                ...
                if (BuildCompat.isQ()) {
                    args[1] = KnoxyCoreCore.getHostPkg();
                }
                ...
                if (auth.equals("settings") || auth.equals("media") || auth.equals("telephony")) {
                    ...
                    ProviderInfo providerInfo = KnoxyCoreCore.getBPackageManager().resolveContentProvider((String) auth, GET_META_DATA, BActivityThread.getUserId());
...
//                                        .field("provider")
//                                        .set(new SettingsProviderStub().wrapper((IInterface) provider, KnoxyCoreCore.getHostPkg()));
//                            }
//                        }
                        return null;
                    }
                    ...
                    if (BActivityThread.getAppPid() != -1) {
                        AppConfig appConfig = KnoxyCoreCore.getBActivityManager().initProcess(providerInfo.packageName, providerInfo.processName, BActivityThread.getUserId());
                        if (appConfig.bpid != BActivityThread.getAppPid()) {
                            providerBinder = KnoxyCoreCore.getBActivityManager().acquireContentProviderClient(providerInfo);
                        }
                        args[authIndex] = ProxyManifest.getProxyAuthorities(appConfig.bpid);
                        args[getUserIndex()] = KnoxyCoreCore.getHostUserId();
                    }
    ...
    public static class StartService extends MethodHook {
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            ...
            ResolveInfo resolveInfo = KnoxyCoreCore.getBPackageManager().resolveService(intent, 0, resolvedType, BActivityThread.getUserId());
            if (resolveInfo == null) {
                return method.invoke(who, args);
            }
            ...
            return KnoxyCoreCore.getBActivityManager().startService(intent, resolvedType, requireForeground, BActivityThread.getUserId());
        }
    ...
    public static class StopService extends MethodHook {
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            Intent intent = (Intent) args[1];
            String resolvedType = (String) args[2];
            return KnoxyCoreCore.getBActivityManager().stopService(intent, resolvedType, BActivityThread.getUserId());
        }
    }
    ...
    public static class StopServiceToken extends MethodHook {
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            ...
            KnoxyCoreCore.getBActivityManager().stopServiceToken(componentName, token, BActivityThread.getUserId());
            return true;
        }
    }
    ...
    public static class BindService extends MethodHook {
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            ...
            ResolveInfo resolveInfo = KnoxyCoreCore.getBPackageManager().resolveService(intent, 0, resolvedType, userId);
            if (resolveInfo != null || AppSystemEnv.isOpenPackage(intent.getComponent())) {
                Intent proxyIntent = KnoxyCoreCore.getBActivityManager().bindService(intent,
        ...
        protected boolean isEnable() {
            return KnoxyCoreCore.get().isBlackProcess() || KnoxyCoreCore.get().isServerProcess();
        }
    }
    ...
    public static class UnbindService extends MethodHook {
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            ...
            KnoxyCoreCore.getBActivityManager().unbindService(iServiceConnection.asBinder(), BActivityThread.getUserId());
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            ...
            for (int i = 0; i < intents.length; i++) {
                ...
                switch (type) {
                    case ActivityManagerCompat.INTENT_SENDER_ACTIVITY:
                        ...
                        shadow.setComponent(new ComponentName(KnoxyCoreCore.getHostPkg(), ProxyManifest.getProxyPendingActivity(BActivityThread.getAppPid())));
            ...
            if (invoke != null) {
                ...
                if (packagesForUid.length < 1) {
                    packagesForUid = new String[]{KnoxyCoreCore.getHostPkg()};
                }
                KnoxyCoreCore.getBActivityManager().getIntentSender(invoke.asBinder(), packagesForUid[0], BActivityThread.getCallingBUid());
            }
            return invoke;
        }
    ...
    public static class getPackageForIntentSender extends MethodHook {
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            ...
            return KnoxyCoreCore.getBActivityManager().getPackageForIntentSender(invoke.asBinder());
        }
    }
    ...
    public static class getUidForIntentSender extends MethodHook {
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            IInterface invoke = (IInterface) args[0];
            return KnoxyCoreCore.getBActivityManager().getUidForIntentSender(invoke.asBinder());
        }
    }
    ...
    public static class BroadcastIntent extends MethodHook {
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            ...
            Intent proxyIntent = KnoxyCoreCore.getBActivityManager().sendBroadcast(intent, resolvedType, BActivityThread.getUserId());
            if (proxyIntent != null) {
    ...
    public static class PeekService extends MethodHook {
        ...
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            ...
            IBinder peek = KnoxyCoreCore.getBActivityManager().peekService(intent, resolvedType, BActivityThread.getUserId());
            return peek;
        }
    }
...
//                args[0] = new ComponentName(KnoxyCoreCore.getHostPkg(), ProxyManifest.getProxyService(BActivityThread.getAppPid()));
//            }
//            return method.invoke(who, args);
            return 0;
        }
    ...
    public static class getCurrentUser extends MethodHook {
        @Override
        protected Object hook(Object who, Method method, Object[] args) throws Throwable {
            Object blackBox = BRUserInfo.get()._new(BActivityThread.getUserId(), "KnoxyCore", BRUserInfo.get().FLAG_PRIMARY());
            return blackBox;
        }
    }
