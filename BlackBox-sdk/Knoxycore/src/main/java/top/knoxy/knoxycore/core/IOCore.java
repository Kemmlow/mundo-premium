...
import dev.knoxy.core.utils.TrieTree;
...
public class IOCore {
    ...
    public void enableRedirect(Context context) {
        ...
        try {
            ApplicationInfo packageInfo = KnoxyCoreCore.getBPackageManager().getApplicationInfo(packageName, PackageManager.GET_META_DATA, BActivityThread.getUserId());
            int systemUserId = KnoxyCoreCore.getHostUserId();
            ...
            if (KnoxyCoreCore.getContext().getExternalCacheDir() != null && context.getExternalCacheDir() != null) {
            ...
            if (KnoxyCoreCore.get().isHideRoot()) {
                hideRoot(rule);
            }
            proc(rule);
        } catch (Exception e) {
