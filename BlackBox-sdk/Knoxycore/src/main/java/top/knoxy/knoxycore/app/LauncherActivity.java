...
public class LauncherActivity extends Activity {
    ...
    public static void launch(Intent intent, int userId) {
        ...
        splash.setClass(KnoxyCoreCore.getContext(), LauncherActivity.class);
        ...
        KnoxyCoreCore.getContext().startActivity(splash);
    }
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        PackageInfo packageInfo = KnoxyCoreCore.getBPackageManager().getPackageInfo(packageName, 0, userId);
        if (packageInfo == null) {
            Slog.e(TAG, packageName + " not installed!");
            finish();
            return;
        }
        Drawable drawable = packageInfo.applicationInfo.loadIcon(KnoxyCoreCore.getPackageManager());
        setContentView(R.layout.activity_launcher);
        findViewById(R.id.iv_icon).setBackgroundDrawable(drawable);
        new Thread(() -> KnoxyCoreCore.getBActivityManager().startActivity(launchIntent, userId)).start();
    }
