package dev.knoxy.core.core.system.am;
...
import dev.knoxy.core.core.system.pm.PackageManagerCompat;
import dev.knoxy.core.proxy.ProxyActivity;
import dev.knoxy.core.proxy.ProxyManifest;
import dev.knoxy.core.proxy.record.ProxyActivityRecord;
import dev.knoxy.core.utils.ComponentUtils;
...
public class ActivityStack {
    ...
    public ActivityStack() {
        mAms = (ActivityManager) KnoxyCoreCore.getContext().getSystemService(Context.ACTIVITY_SERVICE);
    }
    ...
    private int startActivityInNewTaskLocked(int userId, Intent intent, ActivityInfo
        ...
        KnoxyCoreCore.getContext().startActivity(shadow);
        return 0;
    }
    ...
    private int realStartActivityLocked(IInterface appThread, Intent intent, String resolvedType,
        ...
        try {
            ...
            BRIActivityManager.get(BRActivityManagerNative.get().getDefault()).startActivity(appThread, KnoxyCoreCore.getHostPkg(), intent,
    ...
    private Intent getStartStubActivityIntentInner(Intent intent, int vpid,
        ...
        try {
            Resources resources = PackageManagerCompat.getResources(KnoxyCoreCore.getContext(), activityInfo.applicationInfo);
            int id;
            ...
            if (windowIsTranslucent) {
                shadow.setComponent(new ComponentName(KnoxyCoreCore.getHostPkg(), ProxyManifest.TransparentProxyActivity(vpid)));
            } else {
                shadow.setComponent(new ComponentName(KnoxyCoreCore.getHostPkg(), ProxyManifest.getProxyActivity(vpid)));
            }
    ...
    public String getCallingPackage(IBinder token, int userId) {
        synchronized (mTasks) {
            ...
            return KnoxyCoreCore.getHostPkg();
        }
    }
    ...
    public ComponentName getCallingActivity(IBinder token, int userId) {
        synchronized (mTasks) {
            ...
            return new ComponentName(KnoxyCoreCore.getHostPkg(), ProxyActivity.P0.class.getName());
        }
    }
