...
import dev.knoxy.core.utils.compat.DexFileCompat;
...
public class NativeCore {
    ...
    public static void dumpDex(ClassLoader classLoader, String packageName) {
        ...
        for (Long cookie : cookies) {
            if (cookie == 0)
                continue;
//            File file = new File(KnoxyCoreCore.get().getDexDumpDir(), packageName);
...
//            dumpDex(cookie, file.getAbsolutePath());
    ...
    public static int getCallingUid(int origCallingUid) {
        ...
        if (origCallingUid == KnoxyCoreCore.getHostUid()) {
