...
public class AppSystemEnv {
    ...
    static {
        ...
        // sSystemPackages.add(KnoxyCoreCore.getHostPkg());
    ...
    public static boolean isBlackPackage(String packageName) {
        if (KnoxyCoreCore.get().isHideRoot() && sSuPackages.contains(packageName)) {
            return true;
        } else if (KnoxyCoreCore.get().isHideXposed() && sXposedPackages.contains(packageName)) {
            return true;
        }
        return false;
    }
