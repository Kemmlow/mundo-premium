...
//                    }
        ...
        try {
            base = KnoxyCoreCore.getContext().getPackageManager().getPackageInfo(p.packageName, flags);
    ...
    public static ApplicationInfo generateApplicationInfo(BPackage p, int flags, BPackageUserState state, int userId) {
        ...
        try {
            baseApplication = KnoxyCoreCore.getPackageManager().getApplicationInfo(KnoxyCoreCore.getHostPkg(), flags);
        } catch (Exception e) {
            return null;
        }
        String sourceDir = p.baseCodePath;
        if (p.applicationInfo == null) {
            p.applicationInfo = KnoxyCoreCore.getPackageManager()
...
//            ApplicationInfo base = KnoxyCoreCore.getContext().getApplicationInfo();
