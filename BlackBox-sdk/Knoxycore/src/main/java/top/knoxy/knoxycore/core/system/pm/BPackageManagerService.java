...
import dev.knoxy.core.core.system.BProcessManagerService;
import dev.knoxy.core.core.system.ISystemService;
import dev.knoxy.core.core.system.ProcessRecord;
...
import dev.knoxy.core.entity.pm.InstalledPackage;
import dev.knoxy.core.utils.AbiUtils;
import dev.knoxy.core.utils.FileUtils;
import dev.knoxy.core.utils.Slog;
import dev.knoxy.core.utils.compat.PackageParserCompat;
...
public class BPackageManagerService extends IBPackageManagerService.Stub implements ISystemService {
    ...
    public ApplicationInfo getApplicationInfo(String packageName, int flags, int userId) {
        ...
        if (Objects.equals(packageName, KnoxyCoreCore.getHostPkg())) {
            try {
                return KnoxyCoreCore.getPackageManager().getApplicationInfo(packageName, flags);
    ...
    public List<ResolveInfo> queryIntentServices(
        ...
        final String resolvedType = intent.resolveTypeIfNeeded(KnoxyCoreCore.getContext().getContentResolver());
    ...
    public PackageInfo getPackageInfo(String packageName, int flags, int userId) {
        ...
        if (Objects.equals(packageName, KnoxyCoreCore.getHostPkg())) {
            try {
                return KnoxyCoreCore.getPackageManager().getPackageInfo(packageName, flags);
            } catch (PackageManager.NameNotFoundException e) {
                e.printStackTrace();
            }
            return null;
    ...
    private InstallResult installPackageAsUserLocked(String file, InstallOption option, int userId) {
        ...
        try {
            ...
            if (option.isFlag(InstallOption.FLAG_URI_FILE)) {
                ...
                InputStream inputStream = KnoxyCoreCore.getContext().getContentResolver().openInputStream(Uri.parse(file));
            ...
            PackageInfo packageArchiveInfo = KnoxyCoreCore.getPackageManager().getPackageArchiveInfo(apkFile.getAbsolutePath(), 0);
            ...
            if (!support) {
                String msg = packageArchiveInfo.applicationInfo.loadLabel(KnoxyCoreCore.getPackageManager()) + "[" + packageArchiveInfo.packageName + "]";
                return result.installError(packageArchiveInfo.packageName,
                        msg + "\n" + (KnoxyCoreCore.is64Bit() ? "The box does not support 32-bit Application" : "The box does not support 64-bit Application"));
            }
            ...
            if (option.isFlag(InstallOption.FLAG_SYSTEM)) {
                aPackage.applicationInfo = KnoxyCoreCore.getPackageManager().getPackageInfo(aPackage.packageName, 0).applicationInfo;
            }
