...
public class GmsCore {
    ...
    private static InstallResult installPackages(Set<String> list, int userId) {
        KnoxyCoreCore blackBoxCore = KnoxyCoreCore.get();
        for (String packageName : list) {
            if (blackBoxCore.isInstalled(packageName, userId)) {
                continue;
            }
            try {
                KnoxyCoreCore.getContext().getPackageManager().getApplicationInfo(packageName, 0);
    ...
    public static boolean isSupportGms() {
        try {
            KnoxyCoreCore.getPackageManager().getPackageInfo(GMS_PKG, 0);
            return true;
    ...
    public static boolean isInstalledGoogleService(int userId) {
        return KnoxyCoreCore.get().isInstalled(GMS_PKG, userId);
    }
}