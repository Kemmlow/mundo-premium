...
public class MethodParameterUtils {
    ...
    public static String replaceFirstAppPkg(Object[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            if (args[i] instanceof String) {
                ...
                if (KnoxyCoreCore.get().isInstalled(value, BActivityThread.getUserId())) {
                    args[i] = KnoxyCoreCore.getHostPkg();
                    return value;
                }
            }
        }
        ...
        for (int i = 0; i < args.length; i++) {
            if (args[i] instanceof Integer) {
                ...
                if (uid == BActivityThread.getBUid()) {
                    args[i] = KnoxyCoreCore.getHostUid();
                }
            }
        }
    }
    ...
    public static String replaceLastAppPkg(Object[] args) {
        ...
        if (index != -1) {
            ...
            if (KnoxyCoreCore.get().isInstalled(pkg, BActivityThread.getUserId())) {
                args[index] = KnoxyCoreCore.getHostPkg();
            }
            return pkg;
        }
        return null;
