...
import dev.knoxy.core.app.dispatcher.AppServiceDispatcher;
import dev.knoxy.core.core.CrashHandler;
import dev.knoxy.core.core.IBActivityThread;
import dev.knoxy.core.core.IOCore;
import dev.knoxy.core.core.NativeCore;
import dev.knoxy.core.core.env.VirtualRuntime;
import dev.knoxy.core.core.system.user.BUserHandle;
import dev.knoxy.core.entity.AppConfig;
import dev.knoxy.core.entity.am.ReceiverData;
...
import dev.knoxy.core.fake.frameworks.BXposedManager;
import dev.knoxy.core.fake.hook.HookManager;
...
import dev.knoxy.core.utils.compat.ContextCompat;
import dev.knoxy.core.utils.compat.StrictModeCompat;
...
public class BActivityThread extends IBActivityThread.Stub {
    ...
    private final Handler mH = KnoxyCoreCore.get().getHandler();
    ...
    public static int getCallingBUid() {
        return getAppConfig() == null ? KnoxyCoreCore.getHostUid() : getAppConfig().callingBUid;
    }
    ...
    public Service createService(ServiceInfo serviceInfo, IBinder token) {
        ...
        try {
            Context context = KnoxyCoreCore.getContext().createPackageContext(
    ...
    public void bindApplication(final String packageName, final String processName) {
        if (Looper.myLooper() != Looper.getMainLooper()) {
            ...
            KnoxyCoreCore.get().getHandler().post(() -> {
    ...
    public synchronized void handleBindApplication(String packageName, String processName) {
        ...
        PackageInfo packageInfo = KnoxyCoreCore.getBPackageManager().getPackageInfo(packageName, PackageManager.GET_PROVIDERS, BActivityThread.getUserId());
        ...
        Object boundApplication = BRActivityThread.get(KnoxyCoreCore.mainThread()).mBoundApplication();
        ...
        try {
            ...
            BRActivityThread.get(KnoxyCoreCore.mainThread())._set_mInitialApplication(mInitialApplication);
            ContextCompat.fix((Context) BRActivityThread.get(KnoxyCoreCore.mainThread()).getSystemContext());
    ...
    public static Context createPackageContext(ApplicationInfo info) {
        try {
            return KnoxyCoreCore.getContext().createPackageContext(info.packageName,
                    Context.CONTEXT_INCLUDE_CODE | Context.CONTEXT_IGNORE_SECURITY);
        } catch (Exception e) {
            e.printStackTrace();
        }
            ...
            for (ProviderInfo providerInfo : provider) {
                try {
                    if (processName.equals(providerInfo.processName) ||
                        ...
                        installProvider(KnoxyCoreCore.mainThread(), context, providerInfo, null);
                    }
                } catch (Throwable ignored) {
                }
            }
    ...
    public void loadXposed(Context context) {
        ...
        if (KnoxyCoreCore.get().isHideXposed()) {
    ...
    public IBinder getActivityThread() {
        return BRActivityThread.get(KnoxyCoreCore.mainThread()).getApplicationThread();
    }
    ...
    public IBinder acquireContentProviderClient(ProviderInfo providerInfo) throws RemoteException {
        ...
        for (String auth : split) {
            ContentProviderClient contentProviderClient = KnoxyCoreCore.getContext()
    ...
    public void finishActivity(final IBinder token) {
        mH.post(() -> {
            Map<IBinder, Object> activities = BRActivityThread.get(KnoxyCoreCore.mainThread()).mActivities();
    ...
    public void handleNewIntent(final IBinder token, final Intent intent) {
        mH.post(() -> {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP_MR1) {
                newIntent = BRReferrerIntent.get()._new(intent, KnoxyCoreCore.getHostPkg());
            } else {
                newIntent = intent;
            }
            Object mainThread = KnoxyCoreCore.mainThread();
            if (BRActivityThread.get(KnoxyCoreCore.mainThread())._check_performNewIntents(null, null) != null) {
    ...
    public void scheduleReceiver(ReceiverData data) throws RemoteException {
        ...
        mH.post(() -> {
            ...
            try {
                ...
                KnoxyCoreCore.getBActivityManager().finishBroadcast(data.data);
    ...
    public static Activity getActivityByToken(IBinder token) {
        Map<IBinder, Object> iBinderObjectMap =
                BRActivityThread.get(KnoxyCoreCore.mainThread()).mActivities();
    ...
    private void onBeforeCreateApplication(String packageName, String processName, Context context) {
        for (AppLifecycleCallback appLifecycleCallback : KnoxyCoreCore.get().getAppLifecycleCallbacks()) {
