...
import dev.knoxy.core.KnoxyCoreCore.getPackageManager
...
import dev.knoxy.corea.bean.InstalledAppBean
...
class AppsRepository {
    ...
    fun previewInstallList() {
        synchronized(mInstalledList) {
            ...
            for (installedApplication in installedApplications) {
                ...
                val isXpModule = KnoxyCoreCore.get().isXposedModule(file)
    ...
    fun getInstalledAppList(
        ...
        synchronized(mInstalledList) {
            val blackBoxCore = KnoxyCoreCore.get()
    ...
    fun getVmInstallList(userId: Int, appsLiveData: MutableLiveData<List<AppInfo>>) {
        ...
        val applicationList = KnoxyCoreCore.get().getInstalledApplications(0, userId)
    ...
    private fun isInstalledXpModule(packageName: String): Boolean {
        KnoxyCoreCore.get().installedXPModules.forEach {
    ...
    fun unInstall(packageName: String, userID: Int, resultLiveData: MutableLiveData<String>) {
        KnoxyCoreCore.get().uninstallPackageAsUser(packageName, userID)
